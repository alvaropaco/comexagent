...
JOB_NAME="${JOB_NAME:-coffee-market-sync}"
SCHEDULE="${SCHEDULE:-every 6 hours}"
CORE_DATA_API_URL="${CORE_DATA_API_URL:-}"
SCHEDULER_SA_NAME="${SCHEDULER_SA_NAME:-market-sync-scheduler}"
...
gcloud run jobs update "${JOB_NAME}" \
  --command /bin/sh \
  --args "-c","curl -fsS -X POST \"$CORE_DATA_API_URL/market/coffee/sync\" -H 'Content-Type: application/json' -H \"x-market-sync-token: $MARKET_SYNC_TOKEN\" -d '{\"sourceUrl\":\"https://comexlive.org/coffee/\"}'"
