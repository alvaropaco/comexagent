...
JOB_NAME="${JOB_NAME:-market-temporal}"
SCHEDULE="${SCHEDULE:-*/5 * * * *}"
CORE_DATA_API_URL="${CORE_DATA_API_URL:-}"
SCHEDULER_SA_NAME="${SCHEDULER_SA_NAME:-market-temporal-scheduler}"

SYMBOLS="${SYMBOLS:-GC=F SI=F HG=F PL=F PA=F CL=F NG=F HO=F RB=F BZ=F ZC=F ZW=F ZS=F ZM=F ZL=F KC=F RC=F CC=F SB=F CT=F OJ=F LE=F GF=F HE=F LBS=F DC=F}"
...
if ! gcloud run jobs describe "${JOB_NAME}" >/dev/null 2>&1; then
  gcloud run jobs create "${JOB_NAME}" \
    ...
    --set-env-vars "CORE_DATA_API_URL=${CORE_DATA_API_URL},SYMBOLS=${SYMBOLS}"
else
...
gcloud run jobs update "${JOB_NAME}" \
  --command /bin/sh \
  --args "-c","TOKEN=\"\$(printf '%s' \"\$MARKET_SYNC_TOKEN\" | tr -d '\\r\\n')\"; for SYMBOL in \$SYMBOLS; do curl -fsS -X POST \"$CORE_DATA_API_URL/market/ticks/ingest\" -H 'Content-Type: application/json' -H \"x-market-sync-token: \$TOKEN\" -d \"{\\\"symbol\\\":\\\"\$SYMBOL\\\"}\" >/dev/null; curl -fsS -X POST \"$CORE_DATA_API_URL/market/indexes/compute\" -H 'Content-Type: application/json' -H \"x-market-sync-token: \$TOKEN\" -d \"{\\\"symbol\\\":\\\"\$SYMBOL\\\"}\" >/dev/null; done"
