...
JOB_NAME="${JOB_NAME:-external-daily-sync}"
SCHEDULE="${SCHEDULE:-0 3 * * *}"
CORE_DATA_API_URL="${CORE_DATA_API_URL:-}"
SCHEDULER_SA_NAME="${SCHEDULER_SA_NAME:-external-sync-scheduler}"
...
if ! gcloud secrets describe external-tool-token >/dev/null 2>&1; then
  echo "Missing Secret Manager secret: external-tool-token" >&2
  exit 1
fi
...
if ! gcloud run jobs describe "${JOB_NAME}" >/dev/null 2>&1; then
  gcloud run jobs create "${JOB_NAME}" \
    ...
    --max-retries 0 \
    --set-secrets "EXTERNAL_TOOL_TOKEN=external-tool-token:latest" \
...
gcloud run jobs update "${JOB_NAME}" \
  --command /bin/sh \
  --args "-c","curl -fsS -X POST \"$CORE_DATA_API_URL/external/daily-sync\" -H \"x-external-tool-token: $EXTERNAL_TOOL_TOKEN\""
